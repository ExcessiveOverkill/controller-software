set script_dir [file dirname [file normalize [info script]]]
set bit_input "$script_dir/controller_firmware/controller_firmware.runs/impl_1/controller_firmware_top.bit"
set bit_output "$script_dir/bitfile.bit"

file copy -force $bit_input $bit_output

# Generating the .bit file is the primary output. Bootgen is optional because
# some environments do not have it available or do not need the .bin image.
set bootgen_path [auto_execok bootgen]
if {$bootgen_path ne ""} {
	set bif_file "$script_dir/convert.bif"
	set bif_chan [open $bif_file w]
	puts $bif_chan "the_ROM_image:"
	puts $bif_chan "{"
	puts $bif_chan "    $bit_output"
	puts $bif_chan "}"
	close $bif_chan

	set old_pwd [pwd]
	cd $script_dir
	exec $bootgen_path -w -arch zynq -image $bif_file -process_bitstream bin
	cd $old_pwd
}